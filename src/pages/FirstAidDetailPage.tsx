...
const FirstAidDetailPage: React.FC = () => {
    ...
    const loadTopic = async () => {
        ...
        // Clean up slug string formatting (handles 'burns' matching to fallback 'burn' key)
        ...
        if (!found && navigator.onLine) {
            try {
                const response = await fetch(`http://localhost:8000/api/v1/firstaid/${slug}`);
                if (response.ok) {
        ...
        // Match fallback structural object if neither indexedDB nor endpoint had records
        if (!found && FALLBACK_DATA[cleanSlug]) {
            found = {
                id: Math.floor(Math.random() * 1000) + 100, // Dynamic mockup ID mapping
    ...
    return (
        <div className="container py-3 text-start">
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                ...
                <span className="badge bg-secondary py-2 px-3">{topic.category}</span>
            </div>
            
            <p className="lead text-secondary mb-4">{topic.description}</p>
            
            {topic.warning_signs && (
                <div className="alert alert-warning border-0 shadow-sm mb-4">
                    <strong><i className="bi bi-exclamation-triangle-fill me-2"></i>Warning Signs:</strong> {topic.warning_signs}
                </div>
            )}
            
            <div className="card mb-4 shadow-sm border-0">
                <div className="card-header bg-danger text-white fw-bold py-3">
                    <i className="bi bi-list-ol me-2"></i>Step-by-Step Instructions
                </div>
                <div className="card-body bg-light">
                    <ol className="mb-0 ps-3">
                        {steps?.map((step: string, i: number) => (
                            <li key={i} className="mb-2 text-dark">{step}</li>
                        ))}
                    </ol>
                </div>
            </div>
                        ...
                        <div className="card-body bg-light">
                            <ul className="mb-0 ps-3">
                                {dos?.map((item: string, i: number) => (
                                    <li key={i} className="mb-2 text-dark">{item}</li>
                                ))}
                            </ul>
                        </div>
                    </div>
            ...
            <button className="btn btn-danger w-100 py-3 mt-2 shadow-sm fw-bold" 
